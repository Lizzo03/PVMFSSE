...
//        }
        ...
        void gen_update_token(std::string op, std::string w, std::string ind, std::string &l, std::string &e) {
            try {
                ...
                tw = gen_enc_token(w);
                srand(std::stoi(std::to_string(uc)+ind+w));
                std::string stc1 = std::to_string(rand()%100000000);
                std::string rw = gen_enc_token(w + "0");
                // generating update pair, which is (l, e)
                //l = Util::H1(tw + std::to_string(uc + 1));
                //e = Util::Xor(op + ind, Util::H2(tw + std::to_string(uc + 1)));
                l = Util::H1(tw + std::to_string(uc + 1)); //u
                //l = Util::H1(tw + std::to_string(uc + 1));
                e = Util::Xor(op + ind, Util::H2(tw + std::to_string(uc + 1)));
                increase_update_time(w);
                // search_time is used to store the proof(hash)
                std::string hash = Util::Xor(sc, Util::H1(ind));
                std::string hash_p = Util::Xor(hash, Util::H1(rw + tw));
                Util::Xor(hash_p, Util::H1(rw + stc1));
                set_search_time(w, hash);
            }
            catch (const CryptoPP::Exception &e) {
...
//            logger::log_benchmark() << " search time (ms): " << search_time << std::endl;
...
//            logger::log(logger::INFO) << " search set result: " << result.size() << std::endl;
        ...
        void verify(const std::string w, const std::string tw, std::unordered_set <std::string> result) {
            std::string rw, hash;
            rw = gen_enc_token(w + "0");
            int ind_len = AES::BLOCKSIZE / 2; // AES::BLOCKSIZE = 16
            byte tmp[ind_len] = {0};
            std::string Hw = /*Util.str2hex*/(std::string((const char *) tmp, ind_len));
            hash = Util::Xor(Hw, Util::H1(rw + "00000000"));
            hash = Util::Xor(hash, Util::H1(rw + "00000000"));
            //std::string((const char *) tmp, ind_len);
            //std::cout << tmp << std::endl;
            for (std::unordered_set<std::string>::iterator i = result.begin(); i != result.end(); i++) {
                Hw = Util::Xor(Hw, Util::H1(*i));
            }
