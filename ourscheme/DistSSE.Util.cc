...
namespace DistSSE {
    ...
    std::string Util::padding(const std::string str) {
        ...
        std::cout << "padding----pad_len: " + std::to_string(pad_len) << std::endl;
...
*/
    std::string Util::byte2str(unsigned char b[], size_t n) {
        char *p = new char[n];
        memcpy(p, b, n);
        p[n] = 0;
        std::string str(p);
    ...
    unsigned char *Util::str2byte(std::string str) {
    ...
    std::string Util::hex2str(const std::string &input) {
        ...
        for (size_t i = 0; i < len; i += 2) {
            ...
            if (*p != a) throw std::invalid_argument("not a hex digit: " + input);

            char b = input[i + 1];
            const char *q = std::lower_bound(lut, lut + 16, b);
            if (*q != b) throw std::invalid_argument("not a hex digit: " + input);
